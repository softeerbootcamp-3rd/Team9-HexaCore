...
public class PaymentManager {
    ...
    /* 빌링 결제(자동 결제) 요청 */
    public TossPaymentResponse confirmBilling(Integer amount, String orderName, String customerKey, String billingKey) {
        String encodedCredentials = getEncodedCredentials();
        HttpHeaders headers = new HttpHeaders();
        headers.add("Authorization", "Basic " + encodedCredentials);
        ...
        HttpEntity<TossConfirmRequest> requestEntity = new HttpEntity<>(
                TossConfirmRequest.builder().amount(amount).customerKey(customerKey).orderId(orderId).orderName(orderName).build(), headers);
        try {
